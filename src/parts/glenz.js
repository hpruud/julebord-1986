// Glenz vectors: translucent additive-blended filled triangles for the
// classic late-80s/early-90s look. Three nested shapes (cube, octahedron,
// icosahedron) tumble on different axes and combine through additive blending,
// painted from the glenz palette and outlined with glowing edge wireframes.
// Geometry has been beefed up vs. the original two-shape version: the
// icosahedron contributes 20 triangular faces and 30 edges, giving the
// crossover regions much more visual density.
import { createProgram, VW, VH } from '../gl/context.js';
...
void main() {
  ...
  gl_Position = cs;
  v_face = a_face;
  // Normalised "how close to camera" measure for subtle face shading.
...
uniform float u_palLo;   // narrow palette band lower bound
uniform float u_palHi;   // narrow palette band upper bound
void main() {
  // Cycle palette entry per-face within a narrow band so each shape stays in
  // its own hue zone (additive overlap then yields rich colour mixes instead
  ...
  // Faint depth tint -- closer faces a hair brighter so silhouettes pop.
  // Capped at 1.0 so close faces never push above palette colour.
  col *= mix(0.80, 1.00, v_depth);
  // Premultiplied alpha output: blendFunc(ONE, ONE_MINUS_SRC_ALPHA) gives a
  // proper layered translucent ("glenz") look without additive saturation.
  outColor = vec4(col * u_alpha, u_alpha);
}
`;

// Edge wireframe shader (lines).
...
uniform float u_alpha;
// Premultiplied alpha to match the fill program's blend mode.
...
// Regular icosahedron: 12 verts, 20 triangular faces, 30 edges.
// Verts on the three orthogonal "golden" rectangles +/- (1, phi).
function buildIcosahedron() {
  ...
  // 20 faces (standard icosahedron face list).
  const faces = [
  ...
  // Build unique edges (sorted-pair set) from the face list.
  const seen = new Set();
  const edges = [];
  for (const f of faces) {
    const [ia, ib, ic] = f;
...
export function glenz(gl) {
  ...
  gl.linkProgram(prog);
  if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) console.error('glenz link', gl.getProgramInfoLog(prog));
  ...
  return {
    render(gl, t, fbo) {
      bindFBO(gl, fbo);
      // Very dark cool background so the warm reds pop.
      gl.clearColor(0.02, 0.03, 0.06, 1);
      gl.clear(gl.COLOR_BUFFER_BIT);
      gl.enable(gl.BLEND);
      // Premultiplied-alpha blending: the fragment shader emits (col*alpha, alpha)
      // so this gives a proper layered translucent "glenz" look without the
      // colour-clipping that pure additive (ONE, ONE) suffers when 3 shapes overlap.
      gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
      gl.disable(gl.DEPTH_TEST);

      const tt = t / 1000;
      // Wider FOV + slightly closer camera so the shapes fill more of the screen.
      const proj = mat4Mul(mat4FlipY(), mat4Perspective(1.25, VW / VH, 0.1, 100));

      // Camera dolly so shapes pump in/out.
      const camZ = -3.6 - 0.5 * Math.sin(tt * 0.55);
      // Soft alpha breath so the additive blend doesn't blow out to pure white.
      ...
      // ---- Cube (largest, slowest tumble) ----
      {
        ...
        // Reds: palette 0.00..0.22 (deep red -> bright christmas red).
      ...
      // ---- Octahedron (medium, opposite spin) ----
      {
        ...
        gl.uniformMatrix4fv(uMvp, false, mvp);
        gl.uniform1f(uAlpha, 0.55 + 0.10 * breath);
        // Greens: palette 0.78..1.00 (light pine -> deep pine green).
      ...
      // ---- Icosahedron (innermost, fastest spin, third axis) ----
      {
        const r = mat4Mul(mat4RotateY(tt * 1.15), mat4Mul(mat4RotateX(tt * 0.95), mat4RotateZ(-tt * 0.6)));
        // Mid-sized inner shape now that premultiplied alpha prevents the
        // centre from blowing out (no more additive saturation issues).
        ...
        // Warm gold band: palette 0.28..0.40 (bright red -> warm cream).
        // This adds a tinsel-gold sparkle in the centre instead of bleaching
        // the red+green overlap to pure white.
